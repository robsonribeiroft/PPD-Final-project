package com.robsonribeiro.ppd_project_broker.helper
...
@Stable
data class ValidationClientIdResult(
...
object NetworkInputValidator {
    ...
    fun validateClientId(input: String): ValidationClientIdResult {
        if (input.isEmpty()) {
            return ValidationClientIdResult(
                isValid = false,
                errorMessage = "Nickname can't be empty"
            )
        }
        return ValidationClientIdResult(isValid = true)
    }

    fun validateTopicName(input: String): Boolean {
